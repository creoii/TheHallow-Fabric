...
public class BlockSpikeFeature extends Feature<BlockSpikeFeatureConfig> {
    ...
    public boolean generate(FeatureContext<BlockSpikeFeatureConfig> context) {
        ...
        if (!canGenerate(structureWorldAccess, blockPos)) {
            ...
            Optional<CaveSurface> optional = CaveSurface.create(structureWorldAccess, blockPos, blockSpikeFeatureConfig.floorToCeilingSearchRange(), DripstoneHelper::canGenerate, this::canReplaceOrLava);
            if (optional.isPresent() && optional.get() instanceof CaveSurface.Bounded bounded) {
                if (bounded.getHeight() < 4) {
                    ...
                    SpikeGenerator spikeGenerator = createGenerator(blockPos.withY(bounded.getCeiling() - 1), false, random, k, blockSpikeFeatureConfig.stalactiteBluntness(), blockSpikeFeatureConfig.heightScale());
                    SpikeGenerator spikeGenerator2 = createGenerator(blockPos.withY(bounded.getFloor() + 1), true, random, k, blockSpikeFeatureConfig.stalagmiteBluntness(), blockSpikeFeatureConfig.heightScale());
                    WindModifier windModifier2;
                    if (spikeGenerator.generateWind(blockSpikeFeatureConfig) && spikeGenerator2.generateWind(blockSpikeFeatureConfig)) {
                        windModifier2 = new WindModifier(blockPos.getY(), random, blockSpikeFeatureConfig.windSpeed());
                    } else {
                        windModifier2 = WindModifier.create();
                    }

                    boolean bl = spikeGenerator.canGenerate(structureWorldAccess, windModifier2);
                    boolean bl2 = spikeGenerator2.canGenerate(structureWorldAccess, windModifier2);
                    if (bl) {
                        spikeGenerator.generate(structureWorldAccess, random, windModifier2, blockSpikeFeatureConfig);
                    }

                    if (bl2) {
                        spikeGenerator2.generate(structureWorldAccess, random, windModifier2, blockSpikeFeatureConfig);
                    }
    ...
    public boolean canReplaceOrLava(BlockState state) {
        return canReplace(state) || state.isOf(Blocks.LAVA);
    }

    public static boolean canReplace(BlockState state) {
        return state.isOf(BlockRegistry.HALLSTONE) || state.isOf(BlockRegistry.TENEBRITE) || state.isOf(BlockRegistry.PETRIFIED_SAND) || state.isOf(BlockRegistry.PETRIFIED_SANDSTONE) || state.isOf(BlockRegistry.MIDNIGHT_MORTIS) || state.isOf(BlockRegistry.DUSK_MORTIS) || state.isOf(BlockRegistry.DAWN_MORTIS) || state.isOf(BlockRegistry.NOON_MORTIS);
    }

    private static SpikeGenerator createGenerator(BlockPos pos, boolean isStalagmite, Random random, int scale, FloatProvider bluntness, FloatProvider heightScale) {
        return new SpikeGenerator(pos, isStalagmite, scale, bluntness.get(random), heightScale.get(random));
    }

    static final class SpikeGenerator {
        ...
        SpikeGenerator(BlockPos blockPos, boolean bl, int i, double d, double e) {
            this.pos = blockPos;
