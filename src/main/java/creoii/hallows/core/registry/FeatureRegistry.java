...
public class FeatureRegistry {
    ...
    public static final ConfiguredFeature<?, ?> TREE_EBONY = Feature.TREE.configure(new TreeFeatureConfig.Builder(
            new SimpleBlockStateProvider(BlockRegistry.EBONY_LOG.getDefaultState()),
            new LargeOakTrunkPlacer(11, 11, 5),
            new SimpleBlockStateProvider(BlockRegistry.EBONY_LEAVES.getDefaultState()),
            new SimpleBlockStateProvider(BlockRegistry.EBONY_SAPLING.getDefaultState()),
            new BlobFoliagePlacer(ConstantIntProvider.create(2), ConstantIntProvider.create(2), 1),
            new TwoLayersFeatureSize(4, 3, 3)
    ).decorators(Arrays.asList(
            new BranchTreeDecorator(BlockRegistry.EBONY_POST.getDefaultState(), 0.8F, 2, 0.4F),
            //new HangingLeavesTreeDecorator(BlockRegistry.HANGING_BLOOD_EBONY_LEAVES.getDefaultState(), 2, 5),
            new JackOLanternTreeDecorator(0.1F)
    )).dirtProvider(new SimpleBlockStateProvider(BlockRegistry.HALLOWED_DIRT.getDefaultState())).build()).spreadHorizontally().applyChance(3);
    public static final ConfiguredFeature<?, ?> TREE_BLOOD_EBONY = Feature.TREE.configure(new TreeFeatureConfig.Builder(
            new SimpleBlockStateProvider(BlockRegistry.EBONY_LOG.getDefaultState()),
            new LargeOakTrunkPlacer(11, 11, 5),
            new SimpleBlockStateProvider(BlockRegistry.BLOOD_EBONY_LEAVES.getDefaultState()),
            new SimpleBlockStateProvider(BlockRegistry.BLOOD_EBONY_SAPLING.getDefaultState()),
            new BlobFoliagePlacer(ConstantIntProvider.create(2), ConstantIntProvider.create(2), 1),
            new TwoLayersFeatureSize(4, 3, 3)
    ).decorators(Arrays.asList(
            new BranchTreeDecorator(BlockRegistry.EBONY_POST.getDefaultState(), 0.8F, 2, 0.4F),
            //new HangingLeavesTreeDecorator(BlockRegistry.HANGING_BLOOD_EBONY_LEAVES.getDefaultState(), 2, 5),
            new JackOLanternTreeDecorator(0.1F)
    )).dirtProvider(new SimpleBlockStateProvider(BlockRegistry.HALLOWED_DIRT.getDefaultState())).build()).spreadHorizontally().applyChance(2);
